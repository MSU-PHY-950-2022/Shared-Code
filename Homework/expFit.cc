...
void expFit() {
  ...
  //Make a canvas to show our data and fit results
...
// Initialize minuit, set initial values etc. of parameters.
  ...
  par[0] = 1.0;            // a guess at the true value.
  stepSize[0] = 0.1;       // take e.g. 0.1 of start value
  minVal[0] = 1e-4;   // if min and max values = 0, parameter is unbounded.  Only set bounds if you really think it's right!
  maxVal[0] = 1e4;
  parName[0] = "tau";
...
// Plot the result.  For this example plot x values as tick marks.
  ...
  func->GetYaxis()->SetTitle("f(#vec{x};#tau)");
